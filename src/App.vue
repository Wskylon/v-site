<template>
  <div id="vzuor-nexus" class="nexus-root" :class="realmState">
    ...
    <div class="layer-0-parchment"></div>
    <div class="micro-noise-overlay"></div>

    <div class="layer-1-corruption">
      <img src="./assets/ink1.jpg" class="ink-asset ink-top-left" />
      <img src="./assets/ink2.jpg" class="ink-asset ink-bottom-right" />
      <img src="./assets/tentacle1.jpg" class="ink-asset sketch-bg-left" />
      <img src="./assets/ink3.jpg" class="ink-asset hand-of-void" />
    </div>
    ...
    <div class="layer-3-altar">
      <div class="altar-container" :class="{'heartbeat': isHyper}">
        <img src="./assets/totem.jpg" class="crucifixion-totem" />
        ...
        <img src="./assets/ink4.jpg" class="ink-asset blood-splash-left" :class="{'active': isHyper}" />
        <img src="./assets/ink5.jpg" class="ink-asset blood-splash-right" :class="{'active': isHyper}" />
      </div>
    </div>
    ...
    <div class="layer-4-order-grid">
      ...
      <header class="grid-cell header-zone">
        ...
        <div class="time-block">
          ...
          <div class="doom-label">DESTINY_RECKONING (2028-12)</div>
        </div>
      </header>
      ...
      <aside class="grid-cell left-zone">
        ...
        <div class="asset-showcase">
           <img src="./assets/tentacle2.jpg" class="ink-asset mini-sketch" />
...
const processRitual = () => {
  ...
  switch (rootCmd) {
    ...
    case 'help':
      ...
      appendLog(' - vzuor       : 呼唤真名', 'alert')
      break

    default:
      karmaWeight.value += 0.5
      saveMem('vzuor_karma_v4', karmaWeight.value)
      appendLog(`未知频率。业障增加。古神在注视你。`, 'error')
  }
  rawCommand.value = ''
}
...
const triggerHyperMode = () => {
  ...
  appendLog('CRITICAL: 血液开始沸腾。', 'alert')
  ...
  setTimeout(() => {
    ...
    appendLog('机体恢复 3-ON-2-OFF 循环。', 'info')
  }, 8000)
}
...
const bootRenderEngine = () => {
  ...
  const tentacles = []
  for(let i=0; i<8; i++) tentacles.push(new IKTentacle(w/2, h/2, 15, 20, 'rgba(10, 10, 10, 0.4)'))
  const particles = []
  ...
  const loop = () => {
    ...
    particles.forEach(p => { p.update(isEngineHyper); p.draw(ctx, isEngineHyper) })
...
onMounted(() => {
  ...
  bootTimeEngine()
  setTimeout(bootRenderEngine, 500) 
})
...
.nexus-root { position: relative; width: 100%; height: 100%; }

/* 通过 CSS 绑定静态背景资源，绝不会引发 Vite 的解析错误 */
.layer-0-parchment {
...
.sketch-bg-left { top: 20%; left: 5%; width: 40vw; opacity: 0.15; filter: contrast(1) grayscale(100%); } 
...
.layer-2-canvas { position: absolute; inset: 0; z-index: 10; pointer-events: none; mix-blend-mode: multiply; }
...
.altar-container { position: relative; width: 450px; height: 600px; display: flex; justify-content: center; align-items: center; }
.crucifixion-totem {
...
.siphon-core { position: absolute; top: 25%; display: flex; flex-direction: column; align-items: center; mix-blend-mode: normal; }
.core-ring {
...
.core-label { margin-top: 15px; font-size: 1rem; font-weight: bold; letter-spacing: 2px; text-shadow: 0 0 5px rgba(255,255,255,0.8); }
...
/* 第四层：绝对网格 UI */
.layer-4-order-grid {
  position: absolute; inset: 0; z-index: 50; padding: 3rem; box-sizing: border-box;
  display: grid;
...
.zone-title { font-size: 1.2rem; font-weight: bold; border-bottom: 1px solid rgba(0,0,0,0.2); padding-bottom: 5px; margin-bottom: 1.5rem; letter-spacing: 2px; }
...
.sketch-caption { font-size: 0.8rem; font-weight: bold; margin: 10px 0 0 0; color: var(--c-text-muted); }
...
.cli-input { flex: 1; background: transparent; border: none; outline: none; font-size: 1.8rem; font-weight: bold; color: var(--c-text-main); letter-spacing: 1px; }
.cli-input::placeholder { color: var(--c-text-muted); opacity: 0.5; font-style: italic; font-size: 1rem; letter-spacing: 0; }
